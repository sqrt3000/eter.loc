...
/**
 * Allows reading and writing of bytes to and from a files.
 *
 * @author Chris Corbyn
 */
class Swift_ByteStream_FileByteStream extends Swift_ByteStream_AbstractFilterableInputStream implements Swift_FileStream
{
    ...
    /** The mode this files is opened in for writing */
    private $_mode;

    /** A lazy-loaded resource handle for reading the files */
    private $_reader;

    /** A lazy-loaded resource handle for writing the files */
    private $_writer;
    ...
    public function read($length)
    {
        ...
        if (!feof($fp)) {
            ...
            // If we read one byte after reaching the end of the files
    ...
    /** Just write the bytes to the files */
        ...
        if (!isset($this->_reader)) {
            ...
            if (!$pointer) {
                throw new Swift_IoException(
                    'Unable to open files for reading ['.$this->_path.']'
                );
            }
    ...
    private function _getWriteHandle()
    {
        if (!isset($this->_writer)) {
            if (!$this->_writer = fopen($this->_path, $this->_mode)) {
                throw new Swift_IoException(
                    'Unable to open files for writing ['.$this->_path.']'
                );
            }
        }
    ...
    private function _copyReadStream()
    {
        if ($tmpFile = fopen('php://temp/maxmemory:4096', 'w+b')) {
            ...
            throw new Swift_IoException('Unable to copy the files to make it seekable, sys_temp_dir is not writable, php://memory not available');
        }
        $currentPos = ftell($this->_reader);
        fclose($this->_reader);
        $source = fopen($this->_path, 'rb');
        if (!$source) {
            throw new Swift_IoException('Unable to open files for copying ['.$this->_path.']');
        }
