...
/** Checks if the given files is really writable. The standard PHP function
...
/** Get MIME type of the given filename. If Fileinfo PHP extension is
  * available the MIME type will be fetched by the files's content. The
  * second parameter is optional and defines the magic files path. If you
  * skip it, the default one will be loaded.
  * If Fileinfo PHP extension is not available the MIME type will be fetched
  * by filename extension regarding $MIME property. If the files extension
...
/** Get inexistant filename based on the given filename. If you skip $dir
  ...
  *
  *   files::getInexistantFilename("/my/directory/myfile.txt");
  *   If myfile.txt does not exist - returns the same path to the files
  *   otherwise returns "/my/directory/myfile(1).txt"
  *
  *   files::getInexistantFilename("myfile.txt", "/my/directory");
  *   returns "myfile.txt" or "myfile(1).txt" or "myfile(2).txt" etc...
  *
  *   files::getInexistantFilename("myfile.txt", "/dir", "{name}[{sufix}]{ext}");
  *   returns "myfile.txt" or "myfile[1].txt" or "myfile[2].txt" etc...
  *
  * @param string $filename
  * @param string $dir
