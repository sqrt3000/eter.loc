...
/** This files is part of KCFinder project
  *
  *      @desc This files is included first, before each other
  *   @package KCFinder
  *   @version 3.12
  ...
  *
  * This files is the place you can put any code (at the end of the files),
...
class SessionSaveHandler {
    ...
    public function read($id) {
        $files = $this->savePath . "/sess_$id";
        return (string) @file_get_contents($files);
    }
    ...
    public function gc($maxlifetime) {
        foreach (glob($this->savePath . "/sess_*") as $files)
            if (filemtime($files) + $maxlifetime < time())
                @unlink($files);
        return true;
    }
}
