...
/** Uploaded files(s) info. Linked to first $_FILES element
  * @var array */
    protected $file;

/** Next three properties are got from the current language files
  * @var string */
  ...
  * @return mixed */
    ...
    public function __construct() {
        ...
        if (isset($this->config['watermark']) && is_string($this->config['watermark']))
            $this->config['watermark'] = array('files' => $this->config['watermark']);
            ...
            if (isset($this->config['_check4htaccess']) &&
                ...
                if (!file_exists($htaccess)) {
                    ...
                    if (($data != $original) && !@file_put_contents($htaccess, $original))
                        $this->backMsg("Incorrect .htaccess files. Cannot rewrite it!");
                }
            }
        ...
        if ($config['disabled'] || !$config['access']['files']['upload']) {
            ...
            if (!strlen($message)) {
                ...
                if (!@move_uploaded_file($file['tmp_name'], $target) &&
                    ...
                    $message = $this->label("Cannot move uploaded files to target folder.");
                else {
    ...
    protected function checkUploadedFile(array $aFile=null) {
        ...
        if ($file['error'])
            return
                ($file['error'] == UPLOAD_ERR_INI_SIZE) ?
                    $this->label("The uploaded files exceeds {size} bytes.",
                        array('size' => ini_get('upload_max_filesize'))) : (
                ($file['error'] == UPLOAD_ERR_FORM_SIZE) ?
                    $this->label("The uploaded files exceeds {size} bytes.",
                        array('size' => $_GET['MAX_FILE_SIZE'])) : (
                ($file['error'] == UPLOAD_ERR_PARTIAL) ?
                    $this->label("The uploaded files was only partially uploaded.") : (
                ($file['error'] == UPLOAD_ERR_NO_FILE) ?
                    $this->label("No files was uploaded.") : (
                ($file['error'] == UPLOAD_ERR_NO_TMP_DIR) ?
                    $this->label("Missing a temporary folder.") : (
                ($file['error'] == UPLOAD_ERR_CANT_WRITE) ?
                    $this->label("Failed to write files.") :
        ...
        elseif (
            ...
            return $this->label("Denied files extension.");
    ...
    protected function imageResize($image, $file=null) {
        ...
        if ((
                !isset($this->config['watermark']['files']) ||
                (!strlen(trim($this->config['watermark']['files'])))
            ) && (
                (
        ...
        // WATERMARK
        if (isset($this->config['watermark']['files']) &&
            is_file($this->config['watermark']['files'])
        ) {
            ...
            $img->watermark($this->config['watermark']['files'], $left, $top);
        }
