...
class browser extends uploader {
    ...
    protected function act_thumb() {
        if (!isset($_GET['files']) ||
            !isset($_GET['dir']) ||
            !$this->checkFilename($_GET['files'])
        )
            $this->sendDefaultThumb();

        $dir = $this->getDir();
        $file = "{$this->thumbsTypeDir}/{$_GET['dir']}/${_GET['files']}";
    ...
    protected function act_newDir() {
        ...
        if (file_exists("$dir/$newDir"))
            $this->errorMsg("A files or folder with that name already exists.");
    ...
    protected function act_download() {
        ...
        if (!isset($_POST['dir']) ||
            !isset($_POST['files']) ||
            !$this->checkFilename($_POST['files']) ||
            (false === ($file = "$dir/{$_POST['files']}")) ||
        ...
        header('Content-Disposition: attachment; filename="' . str_replace('"', "_", $_POST['files']) . '"');
    ...
    protected function act_rename() {
        ...
        if (isset($this->config['denyExtensionRename']) &&
            ...
            (file::getExtension($_POST['files'], true) !==
        ...
        if (!strlen($newName))
            $this->errorMsg("Please enter new files name.");
        if (preg_match('/[\/\\\\]/s', $newName))
            $this->errorMsg("Unallowable characters in files name.");
        if (substr($newName, 0, 1) == ".")
        ...
        if (!$this->validateExtension($ext, $this->type))
            $this->errorMsg("Denied files extension.");
        if (!@rename($file, $newName))
            $this->errorMsg("Unknown error.");

        $thumbDir = "{$this->thumbsTypeDir}/{$_POST['dir']}";
        $thumbFile = "$thumbDir/{$_POST['files']}";
    ...
    protected function act_delete() {
        ...
        $thumb = "{$this->thumbsTypeDir}/{$_POST['dir']}/{$_POST['files']}";
    ...
    protected function act_cp_cbd() {
        ...
        foreach($_POST['files'] as $file) {
            ...
            $base = basename($file);
            $replace = array('files' => $this->htmlData($base));
            $ext = file::getExtension($base);
            if (!file_exists($path))
                $error[] = $this->label("The files '{files}' does not exist.", $replace);
            ...
            elseif (!$this->validateExtension($ext, $type))
                $error[] = $this->htmlData($base) . ": " . $this->label("Denied files extension.");
            elseif (file_exists("$dir/$base"))
                $error[] = $this->htmlData($base) . ": " . $this->label("A files or folder with that name already exists.");
            elseif (!is_readable($path) || !is_file($path))
                $error[] = $this->label("Cannot read '{files}'.", $replace);
            elseif (!@copy($path, "$dir/$base"))
                $error[] = $this->label("Cannot copy '{files}'.", $replace);
            else {
    ...
    protected function act_mv_cbd() {
        ...
        foreach($_POST['files'] as $file) {
            ...
            elseif (!file::isWritable($path) || !@rename($path, "$dir/$base"))
                $error[] = $this->label("Cannot move '{files}'.", $replace);
            else {
                if (function_exists("chmod"))
                    @chmod("$dir/$base", $this->config['filePerms']);
                $fromThumb = "{$this->thumbsDir}/$file";
    ...
    protected function act_rm_cbd() {
        ...
        foreach($_POST['files'] as $file) {
            ...
            if (!is_file($path))
                $error[] = $this->label("The files '{files}' does not exist.", $replace);
            elseif (!@unlink($path))
                $error[] = $this->label("Cannot delete '{files}'.", $replace);
            else {
    ...
    protected function moveUploadFile($file, $dir) {
        ...
        if (!@move_uploaded_file($file['tmp_name'], $target) &&
            ...
            @unlink($file['tmp_name']);
            return $this->htmlData($file['name']) . ": " . $this->label("Cannot move uploaded files to target folder.");
    ...
    protected function getFiles($dir) {
        ...
        $return = array();
        $files = dir::content($dir, array('types' => "files"));
        if ($files === false)
            return $return;
